...
import com.therouter.apt.transformNumber
...
class TheRouterSymbolProcessor(
    ...
    override fun process(resolver: Resolver): List<KSAnnotated> {
        ...
        genServiceProviderFile(providerItemList, flowTaskList)
        visited = true
        return emptyList()
    }
            ...
            property.annotations.forEach { annotation ->
                ...
                annotation.arguments.forEach { arg ->
                    when (arg.name?.asString()) {
                        ...
                        "id" -> autowiredItem.id = "${arg.value}".toInt()
                        "required" -> {
    ...
    private fun genAutowiredFile(pageMap: Map<String, List<AutowiredItem>>) {
        ...
        for (key in keyList) {
            ...
            try {
                ...
                ps.println()
                ps.println("\t@JvmStatic")
                ps.println(String.format("\tfun autowiredInject(target: %s) {", key))
